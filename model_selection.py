...
def select_page():
        ...
        duration_ms = st.slider('Продолжительность', min_value=0.0, max_value=4000.0, value=500.0, step=0.01)
        ...
        if st.button('Предсказать'):
            ...
            # prediction_genre_clean = prediction
            prediction_genre_clean = prediction[0].replace('[', '').replace(']', '').replace("'", "")
